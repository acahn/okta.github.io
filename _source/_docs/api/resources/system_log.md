...
| -------   | --------------------------------------------------------------------- | ---------------------------------------------------------------| -------- | ------ | -------- | --------- | --------- |
| uuid      | Unique identifier for an indvidiual event                             | String                                                         | FALSE    | TRUE   | TRUE     |           |           |
...
> The `authenticationContext.externalSessionId` can identify multiple requests.  A single `transaction.id` can identify multiple events.  Use the `authenticationContext.externalSessionId` to link events and requests that occurred in the same session.
